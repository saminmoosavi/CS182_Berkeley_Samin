...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions, **kwargs):
        ...
        # loss_fn = nn.MSEloss
        # loss = loss_fn(predicted_action,actions)
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(
        ...
        log_prob = log_prob.sum(dim=-1)  # sum over action dimensions if multi-dim

        # Loss: negative log likelihood (maximize prob of expert actions)
        loss = -log_prob.mean()
        """
        END CODE
        """
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None,
        ...
        if self.nn_baseline:
            ...
            baseline_loss = self.baseline_loss(pred.squeeze(),targets_n)
            """
            END CODE
            """
