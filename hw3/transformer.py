...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        pos_embed_deviced = position_embedding.to(device)
        # print(get_device())
        return  pos_embed_deviced + inputs # add the embedding to the input   #during traning
        # return  position_embedding + inputs # add the embedding to the input   # during eval
...
class TransformerDecoder(nn.Module):
    ...
    def get_future_mask(self, batch_size, sequence_length):
        ...
        return mask.to(device)  # during training
        # return mask               # during eval
...
class Transformer(nn.Module):
    ...
    def forward(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True, shift_target_sequence_right=True):
        ...
        decoder_output = self.decoder(target_sequence, encoder_output,encoder_mask=encoder_mask,decoder_mask=decoder_mask, mask_future=mask_future,shift_target_sequence_right=shift_target_sequence_right)
